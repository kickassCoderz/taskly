...
module.exports = async (req, res) => {
    try {
        ...
        const repository = await fetch(`https://api.github.com/repositories/${webhook.resourceId}`, {
            method: 'GET',
        ...
        while (true) {
            const issues_page = await fetch(
                `https://api.github.com/repos/${repository.full_name}/issues?page=${page}&per_page=100&state=open`,
                {
                    method: 'GET',
                    headers: { authorization: `token ${providerAccessToken}` }
                }
